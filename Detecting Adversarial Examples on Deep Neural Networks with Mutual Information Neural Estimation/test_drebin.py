...
def get_testing_data(x,x_noise,x_adv,y,classifier):
    ...
    return x,x_noise,x_adv,y   # 数据过滤逻辑：x与x_noise target均能正确分类，adv无法正确分类(对抗样本攻击成功)的数据集
...
def get_encoder(encoder_weights,fc_model_weights,vector_dimension):
    encoder = models.get_drebin_encoder()
    fc_model = models.get_drebin_full_connection_layers(vector_dimension)
    encoder.load_weights('networks/drebin/autoencoder/'+encoder_weights+'.h5')
    fc_model.load_weights('networks/drebin/autoencoder/'+fc_model_weights+'.h5')
    ...
    return model
def train_classifier(encoder_name,fc_model_name,savefile, dataset_name):
    vector_dimension = 64
    ...
    x_adv = np.load('{}{}.npy'.format('./adv_data/', 'mama_family_testCW_data'))
    x_adv = np.reshape(x_adv, (-1, 11, 11, 1))
    y_adv = np.load('{}{}.npy'.format('./adv_data/', 'mama_family_testCW_label'))
    ...
    classifier_d.fit(vectors,y_train,batch_size=256,epochs=100,validation_data=(x_adv_vectors,y_adv),callbacks=train_callbacks)   # todo: 把对抗样本作为验证集，以此来调节学习率

    classifier_d.evaluate(x_vectors,y_test,verbose=2)

    classifier_d.save_weights('./networks/' + dataset_name + '/autoencoder/' +savefile+'.h5')

def test_drebin(encoder_name, fc_model_name, simple_classifier_name, classifier):
    vector_dimension = 64

    # classifier = models.get_mnist_local()

    _, _, x_test, y_test = data.load_drebin_data()

    classifier_d = get_classifier(0, vector_dimension)

    classifier_d.load_weights('./networks/drebin/autoencoder/'+simple_classifier_name+'.h5')
    ...
    for i in range(len(model_names)):
        for j in range(len(adv_methods)):
            ...
            #                                   classifier=classifier)    # todo:关闭了过滤数据
            ...
            negative_number = x_a.shape[0]

            TP = negative_number - x_a_equal_num
            FN = x_a_equal_num
            FP = positive_number - x_equal_num
            TN = x_equal_num
...
def training_simple_classifier_ablations():
    ...
    for ablation in ablations:
        ...
        train_classifier(encoder_name=encoder, fc_model_name=full_connection, savefile=savefile, dataset_name='drebin')
if __name__ == '__main__':
    # training_simple_classifier_ablations()
    classifier = models.get_drebin_local(mode='load', dataset_name='drebin')
    ablations = ['none', 'local', 'global', 'prior', 'local_global', 'local_prior', 'global_prior']
    ...
    for ablation in ablations:
        ...
        test_drebin(encoder_name=encoder, fc_model_name=full_connection, simple_classifier_name=simple_classifier, classifier=classifier)
