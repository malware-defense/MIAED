...
def train_classifier(encoder_name,fc_model_name,savefile, dataset_name):
    ...
    classifier_d.fit(vectors,y_train,batch_size=256,epochs=10,validation_data=(x_vectors,y_test),callbacks=train_callbacks)   # todo: 把对抗样本作为验证集，以此来调节学习率
...
def test_mnist(encoder_name, fc_model_name, simple_classifier_name,classifier):
    ...
    classifier_d.load_weights('./'+simple_classifier_name+'.h5')
    ...
    for i in range(len(model_names)):
        for j in range(len(adv_methods)):
            ...
            x_adv = np.add(x_test, np.random.normal(0, 0.2, x_test.shape))
...
def training_simple_classifier_ablations():
    ...
    for ablation in ablations:
        ...
        train_classifier(encoder_name=encoder, fc_model_name=full_connection, savefile=savefile, dataset_name='mnist')
if __name__ == '__main__':
    # training_simple_classifier_ablations()
    classifier = models.get_mnist_local(mode='load', dataset_name='mnist')
    ablations = ['none', 'local', 'global', 'prior', 'local_global', 'local_prior', 'global_prior']
